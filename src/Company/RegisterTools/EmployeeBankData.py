...
class EmployeeBankData(BankData):
    ...
    def __repr__(self) -> str:
        return "ID: %i, O Metodo de pagamento: %s, O valor a receber(s/ descontos): %.2f, O Valor dos Descontos: %.2f" %(self.id, self.paymentMethod , self.value_to_receive, self.tax_to_discount)
